...
export AbstractKernel, Kernel

"""
    abstract type AbstractKernel end    

A kernel is a function from mapping each element in its domain to a measure.

Kernels are required to satisfy a strict form of type stability. Given a kernel
`κ` and any `x::T` and `y::T` in the domain of `κ`, we require 

    typeof(κ(x)) == typeof(κ(y))
"""
abstract type AbstractKernel end
...
struct Kernel{F,S} <: AbstractKernel
    ...
    Kernel(ops::S, ::Type{T}) where {T,S} = new{Type{T},S}(T, ops)
